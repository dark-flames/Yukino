...
impl IntegerType {
    ...
    fn database_value_variant(&self) -> TokenStream {
        match self {
            IntegerType::Integer(8) => quote::quote! {
            ...
            IntegerType::UnsignedInteger(8) => quote::quote! {
...
impl FieldResolveCell for IntegerResolveCell {
    ...
    fn resolve_entity(&mut self, _entity: &EntityResolveCell) -> Result<FieldResolveStatus, ResolveError> {
        unreachable!()
    }
    ...
    fn column_names(&self) -> Result<Vec<String>, UnresolvedError> {
        self.field_name().map(
            |field_name| {
                vec![self.column.as_ref().map(
                    |column| column.name.clone().unwrap_or_else(|| field_name.clone())
    ...
    fn is_primary_key(&self) -> Result<bool, UnresolvedError> {
        self.is_primary_key.ok_or_else(
            || UnresolvedError::new("Integer Resolve cell")
        )
    }
        ...
        match self.status {
            ...
            _ => Ok(vec![
                ColumnDefinition {
                    ...
                    unique: self.column.as_ref().map(|column| column.unique).unwrap_or(false),
                    auto_increase: self.column.as_ref().map(|column| column.auto_increase).unwrap_or(false),
    ...
    fn convert_to_database_value_token_stream(&self, value_ident: &Ident) -> Result<TokenStream, UnresolvedError> {
        self.ty.as_ref().map(
            |ty| ty.to_database_value_tokens(value_ident)
        ).ok_or_else(
            || UnresolvedError::new("Integer Resolve cell")
        )
    }
    ...
    fn breed(&self, entity_name: String, ident: &Ident, attributes: &[FieldAttribute], field_type: &Type) -> Result<Box<dyn FieldResolveCell>, ResolveError> {
        let ty = match field_type {
            Type::Path(type_path) => {
                match type_path.path.segments.first() {
                    Some(first_segment) => {
                        IntegerType::from_ident(&first_segment.ident)
                    },
                    None => None
                }
            },
        ...
        let column = attributes.iter().filter_map(
            |attr| match attr {
