            ...
            Rule::column_ident => {
                ...
                for inner_pair in pair.into_inner() {
                    match inner_pair.as_rule() {
                        ...
                        _ => return Err(location.error(SyntaxError::UnexpectedPair("any or ident")))
                    }
                }
...
fn test_ident() {
    ...
    let result_1 = Grammar::parse(Rule::column_ident, "*").unwrap().next().unwrap();
    let result_2 = Grammar::parse(Rule::column_ident, "a.b.*").unwrap().next().unwrap();

    assert_eq!(ColumnIdent::from_pair(result_1).unwrap().segments, vec!["*".to_string()]);
    assert_eq!(ColumnIdent::from_pair(result_2).unwrap().segments, vec![
